/**
 * File này định nghĩa các hàm kiểm tra luật chơi cờ vua (chess rules) cho game.
 * Nhiệm vụ:
 * - Kiểm tra trạng thái chiếu (check) và chiếu hết (checkmate) của vua.
 * - Lọc các nước đi hợp lệ, loại bỏ nước đi khiến vua bị chiếu.
 * - Lấy tất cả các nước đi hợp lệ cho toàn bộ quân cờ của một bên.
 * - Hỗ trợ các thao tác kiểm tra luật cờ cho các file khác trong chương trình.
 *
 * Các hàm chính:
 * - isKingInCheck: Kiểm tra vua có bị chiếu không.
 * - isMoveValidAgainstCheck: Lọc nước đi hợp lệ, loại bỏ nước đi khiến vua bị chiếu.
 * - checkForCheckMate: Kiểm tra chiếu hết và hiển thị thông báo chiến thắng.
 * - getAllPossibleMoves: Lấy tất cả nước đi hợp lệ cho một bên.
 */
...
function isMoveValidAgainstCheck(
  ...
  blackKingSquare,
  boardSquaresArray,
  historyMoves // Lịch sử nước đi (để kiểm tra chiếu hết)
) {
  ...
  legalSquaresCopy.forEach((element) => {
    ...
    if (
      pieceType != "king" &&
      isKingInCheck(kingSquare, pieceColor, boardSquaresArrayCopy, historyMoves)
    ) {
