/**
 * moves.js
 * Đảm nhận vai trò xử lý logic các nước đi hợp lệ cho từng loại quân cờ trong game cờ vua.
 * Bao gồm các hàm tính toán nước đi cho tốt, mã, tượng, xe, hậu, vua và các hàm kiểm tra nước ăn chéo, nước đi đặc biệt.
 * Các hàm này không phụ thuộc vào biến toàn cục, chỉ sử dụng tham số truyền vào để đảm bảo tính module hóa.
 */

import { isShortCastlePossible, isLongCastlePossible } from "./castling.js";
...
function getPossibleMoves(
  ...
  if (!Array.isArray(historyMoves)) {
    console.error(
      "historyMoves bị undefined!",
      historyMoves,
      startingSquareId,
      piece
    );
...
function checkPawnDiagonalCaptures(
  ...
  for (let i = -1; i <= 1; i += 2) {
    ...
    if (
      ...
      // --- Kiểm tra en passant ---
      if (currentSquareId === window.enPassantSquare) {
        legalSquares.push(currentSquareId);
      }
    }
  }
...
function getKingMoves(
  ...
  // Bổ sung nước nhập thành vào danh sách nước đi của vua nếu hợp lệ.
  // Nếu vua và xe chưa từng di chuyển, các ô giữa trống, sẽ thêm "g1/g8" (nhập thành ngắn) hoặc "c1/c8" (nhập thành dài)
  // vào legalSquares để cho phép nhập thành.
  let shortCastleSquare = isShortCastlePossible(
    historyMoves,
    pieceColor,
    boardSquaresArray
  );
